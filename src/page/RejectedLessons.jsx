...
const RejectedLessons = () => {
  ...
  const fetchRejectedLessons = useCallback(async (page = 1) => {
    ...
    try {
      ...
      const response = await axios.get(
        `https://teacheraitools-cza4cbf8gha8ddgc.southeastasia-01.azurewebsites.net/api/v1/teacher-lessons?Status=4&userId=${userInfo.id}&Page=${page}&PageSize=${pageSize}`,
        {
          headers: {
  ...
  }, [fetchRejectedLessons, currentPage]); // Use the stable fetch function
  ...
  const handleNavigateToDetail = (lessonId) => {
    if (lessonId) {
      navigate(`/Bài-giảng-đã-từ-chối/${lessonId}`);
    } else {
        ...
        >
          ...
          {loading ? (
            ...
            <>
              <List sx={{ p: 0 }}>
                {lessons.map((lesson) => (
                  <ListItem key={lesson.teacherLessonId} disablePadding>
                    <ListItemButton
                    ...
                    >
                      ...
                      <ListItemText
                        ...
                        secondary={`Chủ đề: ${lesson.module || 'N/A'} | Từ chối lúc: ${formatDate(lesson.createdAt)}`}

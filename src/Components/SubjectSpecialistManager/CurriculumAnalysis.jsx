...
const CurriculumAnalysis = ({ sidebarOpen }) => {
    const [reportsByGrade, setReportsByGrade] = useState([]); // Danh sách reports cho từng khối lớp
    ...
    // Lấy dữ liệu reports cho từng khối lớp
    useEffect(() => {
            ...
            try {
                ...
                const fetchedReports = await Promise.all(
                    grades.map(async (grade) => {
                        // Giả sử API hỗ trợ lấy reports theo grade
    ...
    const handleGradeChange = (event, newValue) => {
        ...
        setProposal(''); // Reset proposal khi chuyển khối lớp
    };
    ...
    const handleSubmitProposal = async () => {
        ...
        try {
            ...
            // Gửi proposal cho khối lớp được chọn
    ...
    // Lấy report hiện tại dựa trên khối lớp được chọn
